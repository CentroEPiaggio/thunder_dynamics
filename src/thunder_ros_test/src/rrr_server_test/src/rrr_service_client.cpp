...
int main(int argc, char* argv[]) {
	...
	try {
        ...
        // fill request
		set_request->value = {distribution(generator), distribution(generator), distribution(generator)};
		...
		auto q_response = wait_for_response(node, get_q->async_send_request(std::make_shared<rrr_server::srv::GetValue::Request>()), "/rrr_server/get_q");
		RCLCPP_INFO(node->get_logger(), "q = [%.3f, %.3f, %.3f]", q_response->value.at(0), q_response->value.at(1), q_response->value.at(2));

		auto m_response = wait_for_response(node, get_M->async_send_request(std::make_shared<rrr_server::srv::GetValue::Request>()), "/rrr_server/get_M");
