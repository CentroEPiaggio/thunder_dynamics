...
int create_thunder_robot(const string robot_name, Robot& robot, const string file_h, const string file_cpp, const bool gen_python){
	...
	if(gen_python){
		string bindings_str = 
			...
			"\tpy::class_<"+thunder_robot_name+">(m, \"thunder_robot\")\n"
			"\t\t.def(py::init<>())\n";
		// - pybind functions - //
		for (auto fun : functions){
	...
	for (auto par : parameters){
		if (par.second.symb_size() != 0){
			...
			functions_str.append("// get the parameter: " + par.second.name + "\n");
			functions_str.append(
				"Vector<double," + std::to_string(out_size) + "> " + thunder_robot_name + "::" + fun_name + "(){\n"
					"\treturn this." + par.second.name + ";\n"
				"}\n");
		}
	}
	...
	for (auto par : parameters){
		if (par.second.symb_size() != 0){
			...
			functions_str.append("// set the parameter: " + par.second.name + "\n");
			functions_str.append(
				"void " + thunder_robot_name + "::" + fun_name + "(Vector<double,"+std::to_string(in_size)+"> value){\n"
					"\tthis." + par.second.name + " = value;\n"
				"}\n");
		}
	}
