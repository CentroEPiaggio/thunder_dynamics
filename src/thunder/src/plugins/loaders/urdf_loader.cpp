...
namespace thunder_ns {
	...
	struct BodyDYN {
		...
		Eigen::Matrix3d Io = Eigen::Matrix3d::Zero();

		void add(const BodyDYN& other, const Eigen::Matrix4d& T) {
			Eigen::Matrix3d R = T.block<3, 3>(0, 0);
			Eigen::Vector3d p = T.block<3, 1>(0, 3);

			double m_other = other.m;
			Eigen::Vector3d mc_other_L = other.mc;
			Eigen::Matrix3d Io_other_L = other.Io;

			m += m_other;
			Eigen::Vector3d Rmc = R * mc_other_L;
			mc += Rmc + m_other * p;
			...
			Eigen::Matrix3d skew_Rmc = skew(Rmc);

			Io += R * Io_other_L * R.transpose() + 
				  skew_p * skew_Rmc.transpose() + 
				  skew_Rmc * skew_p.transpose() + 
				  m_other * skew_p * skew_p.transpose();
		}
	};

	BodyDYN extract_dyn(std::shared_ptr<urdf::Link> link) {
		BodyDYN dyn;
		if (link->inertial) {
			dyn.m = link->inertial->mass;
			Eigen::Matrix4d T_li = link->inertial->origin.matrix();
			Eigen::Matrix3d R_li = T_li.block<3, 3>(0, 0);
			Eigen::Vector3d p_li = T_li.block<3, 1>(0, 3);

			Eigen::Matrix3d Ic_i;
			Ic_i << link->inertial->ixx, link->inertial->ixy, link->inertial->ixz,
				    link->inertial->ixy, link->inertial->iyy, link->inertial->iyz,
				    link->inertial->ixz, link->inertial->iyz, link->inertial->izz;

			Eigen::Matrix3d Ic_l = R_li * Ic_i * R_li.transpose();
			dyn.mc = dyn.m * p_li;
			...
			Eigen::Matrix3d skew_p = skew(p_li);
			dyn.Io = Ic_l + dyn.m * skew_p * skew_p.transpose();
		}
		return dyn;
	}
	...
	std::shared_ptr<Robot> UrdfLoader::load(std::shared_ptr<Robot> robot) {
		...
		try {
			...
			std::vector<BodyDYN> active_bodies;
			
			Eigen::Matrix4d current_static_transform = Eigen::Matrix4d::Identity();
			Eigen::Matrix4d current_link_transform = Eigen::Matrix4d::Identity();
			...
			for (size_t i = 0; i < chain.size() - 1; i++) {
				...
				if (joint->type == urdf::JointType::FIXED) {
					...
					current_static_transform = current_static_transform * joint->parent_to_joint_transform.matrix();
					current_link_transform = current_link_transform * joint->parent_to_joint_transform.matrix();

					if (!active_bodies.empty()) {
						active_bodies.back().add(extract_dyn(child), current_link_transform);
					}
				} else {
					active_joints.push_back(joint);
					// The transform for this joint is current_static_transform * joint->parent_to_joint_transform
					static_transforms.push_back(current_static_transform * joint->parent_to_joint_transform.matrix());
					// Reset accumulated transform
					current_static_transform = Eigen::Matrix4d::Identity();

					// Start new active body
					current_link_transform = Eigen::Matrix4d::Identity();
					active_bodies.push_back(extract_dyn(child));
				}
			}
			...
			for (int i = 0; i < numJoints; ++i) {
				...
				par_DYN_num[STD_PAR_LINK * i + 0] = b.m;
				par_DYN_num[STD_PAR_LINK * i + 1] = b.mc(0);
				par_DYN_num[STD_PAR_LINK * i + 2] = b.mc(1);
				par_DYN_num[STD_PAR_LINK * i + 3] = b.mc(2);
				par_DYN_num[STD_PAR_LINK * i + 4] = b.Io(0, 0); // Ixx
				par_DYN_num[STD_PAR_LINK * i + 5] = b.Io(0, 1); // Ixy
				par_DYN_num[STD_PAR_LINK * i + 6] = b.Io(0, 2); // Ixz
				par_DYN_num[STD_PAR_LINK * i + 7] = b.Io(1, 1); // Iyy
				par_DYN_num[STD_PAR_LINK * i + 8] = b.Io(1, 2); // Iyz
				par_DYN_num[STD_PAR_LINK * i + 9] = b.Io(2, 2); // Izz
			}
